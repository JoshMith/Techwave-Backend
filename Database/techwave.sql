...
-- Complete Database Schema v2.0 (Cleaned - No Redundancy)
-- March 2026
--
-- Changes from v1.0:
...
CREATE TABLE users (
    ...
    password_hash   VARCHAR(255), -- allow NULL for social login users
...
CREATE TABLE carts (
    ...
    created_at      TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at      TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT cart_owner_constraint CHECK (
...
-- Seed commission rates (replace 1 with actual admin user_id):
...
FROM categories WHERE name IN ('Phones', 'Laptops', 'Home Appliances', 'Gaming', 'Audio & Sound');
...
BEGIN
    UPDATE products
    SET
        rating = (SELECT COALESCE(ROUND(AVG(rating)::NUMERIC, 2), 0.00) FROM reviews WHERE product_id = COALESCE(NEW.product_id, OLD.product_id)),
...
CREATE TRIGGER trg_refresh_rating_insert AFTER INSERT ON reviews FOR EACH ROW EXECUTE FUNCTION refresh_product_rating();
CREATE TRIGGER trg_refresh_rating_delete AFTER DELETE ON reviews FOR EACH ROW EXECUTE FUNCTION refresh_product_rating();
...
CREATE TRIGGER trg_order_status_history AFTER UPDATE ON orders FOR EACH ROW EXECUTE FUNCTION log_order_status_change();


-- ============================================================
-- M-PESA TRANSACTION FUNCTIONS
-- ============================================================
...
CREATE TABLE IF NOT EXISTS mpesa_transactions_archive (LIKE mpesa_transactions INCLUDING ALL);
...
SELECT
    ...
    COALESCE(SUM(CASE WHEN cat.name = 'Accessories' THEN c.amount_earned ELSE 0 END), 0.00) AS accessories_commission,
    COALESCE(SUM(CASE WHEN cat.name != 'Accessories' THEN c.amount_earned ELSE 0 END), 0.00) AS electronics_commission,
...
SELECT
    ...
    COUNT(CASE WHEN o.payment_method = 'cash_on_delivery' THEN 1 END) AS cod_orders,
    COUNT(CASE WHEN o.agent_id IS NOT NULL THEN 1 END) AS agent_referred_orders,
    SUM(CASE WHEN o.agent_id IS NOT NULL THEN o.commission_total ELSE 0 END) AS total_commissions_paid
FROM orders o
...
-- Delete abandoned guest carts older than 30 days
DELETE FROM carts 
...
-- ============================================================
-- END OF SCHEMA
-- TechWave Electronics Kenya — Database Schema v2.0 (Cleaned)
-- ============================================================